...
import {
	cache,
	computeMRR,
	computeSubscriptionSumWithDiscount,
	getSubscriptionDiffByMonth,
} from '../src/service'

import { Subscription } from '../src/types'
import {
	mockedHttpResponseForJan22,
	mockedSubscriptionWithPercentOff,
	mockedSubscriptionWithoutPercentOff,
	mockedHttpResponseForFeb22,
} from './data.mock'
...
describe('service.test', () => {
	...
	it('computeMRR for Jan 22 should return 2537.76 USD', async () => {
		vi.mocked(fetchSubscriptions).mockResolvedValueOnce([
			mockedHttpResponseForJan22,
		])
	...
	it('computeMRR for Mar 22 should return 2413.62 EUR', async () => {
		...
		await expect(() => computeMRR('Mar 22', 'eur')).rejects.toThrowError(
			'No subscriptions found for month : Mar 22',
		)
	})
